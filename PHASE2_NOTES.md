...
- **What separated wins from losses — hitting efficiency.** Team hit% is the one clean factor: +.197 in won sets vs +.049 in lost sets (Cohen's d = +0.98). Opponent points (d = −1.15) was originally read as "defense edged offense" but is redundant and excluded: losing a set means the opponent reached the cap by definition (9 of Bean's 10 lost sets had the opponent at 25-26; the 23.2 average only dips below 25 because of one cut-short game 3). Non-redundant defense proxies are weak: digs d = −0.35 (wrong direction), blocks d = +0.26. The data measures Bean's offense well and its defense poorly.
